        ...
        <li class="list-group-item list-group-item-action">
          <div class="btn-group">
            @if($category->name == "Beers")
              <a href="{{ route('shop.beers_index') }}" class="shop_links category_active">{{$category->name}}</a>
          ...
          <div class="row" style="margin-left:-25px;">
            ...
            @else
              @foreach ($beers as $beer)
                <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12 product_col">
                  <div class="card" style="width: 18rem;">
                    <img class="card-img-top" src="{{$beer->img}}" alt="Card image cap">
                    <div class="card-body">
                      <h5 class="card-title">{{$beer->name}}</h5>
                      <p class="card-text">{{$beer->info}}</p>
                      <label>Price: {{$beer->price}}$</label>
                      <br>
                      <label>Type: {{$beer->type}}</label>
                      <br>
                      <label>Brewery: {{$beer->brewery}}</label>
                      <br>
                      <label>ABV: {{$beer->abv}}</label>
                      <br>
                      <a href="{{route('shop.page', ['name' => $beer->name])}}" class="btn btn-primary" style="margin-top:5px;">Check it</a>
                    </div>
                  </div>
                </div>
              @endforeach
